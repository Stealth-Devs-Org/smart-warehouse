...
BROKER = "localhost"  
PORT = 1883
TOPIC = "/actuator_Temperature"

class TemperatureCtrl(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            SetActuatorState("TemperatureController", self.actuator_id,  self.partition_id, self.actuator_id, round(rateofChange, 2), 1)
...
def main():
    no_of_partitions = len(TemperatureCtrlID)
    allActuators = []

    for j in range(no_of_partitions):
        allActuators.append([])
        for coord in TemperatureCtrlID[j]:
            actuator = TemperatureCtrl(actuator_id=coord, partition_id=j)  
