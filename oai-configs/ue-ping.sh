...
print_usage() {
  ...
  echo "  [options] - Additional arguments for ping"
}
...
if ! sudo ip netns exec ue"$ueid" ip link show oaitun_ue1 2>&1; then
  exit 1
fi
...
# Run ping in the UE1 namespace with arguments
sudo ip netns exec ue"$ueid" ping -I oaitun_ue1 $arg_str