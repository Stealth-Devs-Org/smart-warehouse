...
def calculate_averages_by_agv(filename):
    ...
    with open(filename, 'r') as file:
        ...
        for row in reader:
            agv_id, type, t1, t2, t3, t4 = row[0], int(row[1]), float(row[2]), float(row[3]), float(row[4]), float(row[5])
    ...
    for agv_id, diffs in agv_data.items():
        avg_t1_t4 = sum(diffs['t1_t4_diff']) / len(diffs['t1_t4_diff']) if diffs['t1_t4_diff'] else 0
        avg_t2_t3 = sum(diffs['t2_t3_diff']) / len(diffs['t2_t3_diff']) if diffs['t2_t3_diff'] else 0
        avg_final = sum(diffs['final_diff']) / len(diffs['final_diff']) if diffs['final_diff'] else 0
    ...
    avg_overall_t1_t4 = sum(overall_t1_t4_diff) / len(overall_t1_t4_diff) if overall_t1_t4_diff else 0
    avg_overall_t2_t3 = sum(overall_t2_t3_diff) / len(overall_t2_t3_diff) if overall_t2_t3_diff else 0
    avg_overall_final = sum(overall_final_diff) / len(overall_final_diff) if overall_final_diff else 0
...
filename = 'interrupt_response.csv'
